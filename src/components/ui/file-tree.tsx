...
type TreeViewProps = {
  ...
  openIcon?: React.ReactNode;
  closeIcon?: React.ReactNode;
  // MODIFIED: Prop name changed to onSelect and it now passes the whole element.
  onSelect?: (element: TreeViewElement | undefined) => void;
} & TreeViewComponentProps;

// Helper function to find a node by its ID in the tree structure.
const findNodeById = (nodes: TreeViewElement[], id: string): TreeViewElement | undefined => {
...
const Tree = forwardRef<HTMLDivElement, TreeViewProps>(
  (
    {
      className,
      elements = [], // Provide a default value
      initialSelectedId,
      ...
      closeIcon,
      dir,
      onSelect, // MODIFIED: Use the new onSelect prop.
      ...props
    },
    ref,
  ) => {
    ...
    const selectItem = useCallback(
      (id: string) => {
        setSelectedId(id);
        // MODIFIED: Find the full node object and pass it to the onSelect callback.
        if (onSelect) {
          const selectedNode = findNodeById(elements, id);
    ...
    return (
      ...
      >
        <div className={cn("size-full", className)}>
          <ScrollArea ref={ref} className="relative h-full px-2" dir={direction as "ltr" | "rtl"}>
...
const Folder = forwardRef<HTMLDivElement, FolderProps & React.HTMLAttributes<HTMLDivElement>>(
  ({ className, element, value, isSelectable = true, isSelect, children, ...props }, ref) => {
    const {
      ...
      openIcon,
      closeIcon,
      // ADDED: Get selectedId and selectItem from context for selection logic.
      selectedId,
      selectItem,
    } = useTree();

    // ADDED: Determine if the folder is currently selected.
    ...
    return (
      <AccordionPrimitive.Item {...props} value={value} className="relative h-full overflow-hidden">
        <AccordionPrimitive.Trigger
          className={cn(
            `flex items-center gap-1 rounded-md px-1 text-sm`, // Added padding for better clicking area
            className,
            {
              // MODIFIED: Use the new isSelected state to apply background color.
          ...
          onClick={() => {
            // MODIFIED: Call both handleExpand and selectItem on click.
            handleExpand(value);
            selectItem(value);
          }}
        >
  ...
  (
    ...
    return (
      <button
        ...
        disabled={!isSelectable}
        className={cn(
          "flex w-fit items-center gap-1 rounded-md px-1 text-sm duration-200 ease-in-out rtl:pr-0 rtl:pl-1",
          {
